...
def parse(file_contents):
    ...
    patterns = {
        ...
        'gate': re.compile(r'(not|nand|nor|xor)\s+(\w+)\s*\(([^)]+)\);', re.MULTILINE)
    }
    ...
    for gate_match in patterns['gate'].finditer(file_contents):
        ...
        if not any(gate_name.lower().startswith(prefix) for prefix in ['nand', 'nor', 'not', 'xor']): continue
        ...
        if net.source and net.destinations:
            ...
            for dest_gate in net.destinations:
                if source_gate.name != dest_gate.name:
                    is_gate = lambda g: any(g.name.lower().startswith(p) for p in ['nand', 'nor', 'not', 'xor'])
